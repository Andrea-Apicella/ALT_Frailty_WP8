...
class TemplateMatch:
    def __init__(
        ...
        if element_type in ["timestamp", "datalogger"]:
            ...
            print(
                'Specified type of element to extract is invalid. Choose "timestamp" or "datalogger".'
            )
            return
    ...
    def __pre_process_templates(self) -> None:
        ...
        self.templates = list(
            map(lambda template: pre_process(template), self.templates)
        )
