...
class TemplateMatch():
    ...
    def __init__(self, video_path, element_type, timestamp_roi=None, datalogger_roi=None,
                 target_template_size=(174, 255), target_timestamp_size=(2444, 428)):
        self.templates_path = './data/templates'
        try:
            self.templates = [cv2.imread(template)
                              for template in listdir_nohidden_sorted(self.templates_path)]
            print(
                f'Found {len(self.templates)} templates in {self.templates_path}.')
        except Exception:
            print('Can\'t find templatess.')
            traceback.print_exc()
    ...
    def template_match(self, frame, threshold=0.7, test=False):
        ...
        for template_number, _ in enumerate(self.templates):
            ...
            result = cv2.matchTemplate(
                tomatch_copy, curr_template, cv2.TM_CCOEFF_NORMED)
            ...
            if test:
                for (startX, startY, endX, endY) in nms_boxes:
                    cv2.rectangle(tomatch_copy, (startX, startY),
                                  (endX, endY), (0, 0, 0), 3)
    ...
    def extract_timestamps(self):
        ...
        for frame_number in (t := trange(self.total_frames)):
            ...
            if not success:
                ...
                break
            timestamp_extracted = self.template_match(
                frame)
            # print(timestamp_extracted)
    ...
    def __test_single_timestamp(self):
        ...
        timestamp_roi = first_frame[top_y:top_y +
                                    bottom_y, top_x:top_x + bottom_x]
        plt.subplot(1, 2, 1)
        plt.imshow(first_frame)
        plt.subplot(1, 2, 2)
        plt.imshow(timestamp_roi)
        plt.show()
